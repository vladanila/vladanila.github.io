// Global variables
let currentMode = "resize"; // possible values: "resize", "crop", "convert", "filter", "watermark", "collage", "border"
...
document.getElementById("upload").addEventListener("change", function (event) {
  ...
  if (file) {
    ...
    reader.onload = function (e) {
      ...
      // For convert, filter, watermark, collage, or border modes, show preview
      if (
        currentMode === "convert" ||
        currentMode === "filter" ||
...
function applyFilter(filterValue) {
  ...
  gsap.fromTo(imgElement, { scale: 1 }, { duration: 0.3, scale: 1.05, yoyo: true, repeat: 1 });
}
...
function applyWatermark() {
  ...
  img.onload = function() {
    ...
    ctx.font = Math.floor(canvas.width / 20) + "px Poppins";
...
/* Collage: Set up event listeners for each collage upload */
for (let i = 1; i <= 4; i++) {
